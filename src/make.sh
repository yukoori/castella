...
echo -e $Color_Off$BGreen$os "castella build start!!!================"$Color_Off
if [[ $os = 'Linux' ]]
then
...
if [[ -d ../lib ]]
then
	echo -e $Color_Off$On_Yellow$BBlue'output directory already exist.' $Color_Off
else
	mkdir ../lib;
fi
...
then
	...
	$MAKE;
	if [[ $? != 0 ]]; then
		echo -e $Color_Off$On_Red$BYellow'Fail to Compile "castella"\n'$Color_Off
		exit 1
	fi
elif [[ $1 = 'build' ]]
then
...
echo -e $Color_Off$BGreen$os "castella build finished!!!================"$Color_Off
